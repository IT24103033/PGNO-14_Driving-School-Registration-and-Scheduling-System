  ...
  <table class="table table-bordered">
    ...
    <tr>
      ...
      <td>
        ...
        <button type="button" class="btn btn-warning action-btn" data-bs-toggle="modal" data-bs-target="#updateModal<%= i %>">Update</button>

        <!-- Modal for Update -->
        <div class="modal fade" id="updateModal<%= i %>" tabindex="-1" aria-labelledby="updateModalLabel<%= i %>" aria-hidden="true">
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="updateModalLabel<%= i %>">Update Lesson</h5>
              ...
              <div class="modal-body">
                <form action="lesson" method="post">
                  ...
                  <div class="mb-3">
                    ...
                    <input type="date" class="form-control" id="date<%= i %>" name="date" value="<%= lesson.getDate() %>" required>
                  </div>
                  <div class="mb-3">
                    <label for="time<%= i %>" class="form-label">Time</label>
                    <input type="time" class="form-control" id="time<%= i %>" name="time" value="<%= lesson.getTime() %>" required>
                  </div>
                  <div class="mb-3">
                    <label for="lessonType<%= i %>" class="form-label">Lesson Type</label>
                    <select class="form-control" id="lessonType<%= i %>" name="lessonType" required>
                      <option value="Highway Driving" <%= "Highway Driving".equals(lesson.getLessonType()) ? "selected" : "" %>>Highway Driving</option>
                      <option value="Parallel Parking" <%= "Parallel Parking".equals(lesson.getLessonType()) ? "selected" : "" %>>Parallel Parking</option>
                      <option value="City Driving" <%= "City Driving".equals(lesson.getLessonType()) ? "selected" : "" %>>City Driving</option>
                      <option value="Practice Test" <%= "Practice Test".equals(lesson.getLessonType()) ? "selected" : "" %>>Practice Test</option>
                      <option value="Night Driving" <%= "Night Driving".equals(lesson.getLessonType()) ? "selected" : "" %>>Night Driving</option>
                    </select>
                  </div>
